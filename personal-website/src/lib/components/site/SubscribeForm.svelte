...
<script lang="ts">
	...
	// Browser autofill keys off these, so the two fields fill in one tap.
...
<style lang="scss">
	...
	@media (max-width: 480px) {
		...
		// The 10rem basis is a width in a row and a height in a column, so it has
		// to be dropped once the row stacks.
		input {
			flex: 0 0 auto;
		}
